(function () {
  ...
  var forgeMeta = document.querySelector('meta[name="forge-route"]');
  if (forgeMeta) {
    var forgePath = forgeMeta.content;
    var forgeRoot = window.ForgeUI ? window.ForgeUI.root() : new URL("./", location.href);
    var currentPath = new URL(forgePath, forgeRoot).pathname;
    document.querySelectorAll("a.sidebarElem").forEach(function (link) {
      if (link.classList.contains("selected")) return;
      if (new URL(link.href).pathname === currentPath) link.classList.add("selected");
    });
  }
